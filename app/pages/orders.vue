<template>
  <div class="p-6 bg-gray-50 min-h-screen space-y-8">
    ...
    <div v-else>
      ...
      <div
        class="bg-white rounded-xl p-4 mb-6 shadow-sm border border-gray-200"
      >
        ...
        <div v-if="searchQuery" class="mt-2 text-sm text-black">
          Znaleziono:
          <span class="font-semibold">{{ totalFilteredOrders }}</span> zamówień
        </div>
      </div>

      <div v-if="sectionOrderWithData" class="text-black">
        <div class="mb-12" v-for="section in sectionOrderWithData">
          <div
            :class="[
            ...
            v-if="section.orders && section.orders.length > 0"
          >
            <div class="flex items-center justify-between mb-6">
              <div class="flex-items-center gap-3">
                ...
                <h2 v-else :class="['text-3xl font-bold', section.textClass]">{{ section.title }}</h2>
                <OrdersPageWebSocketStatusIndicator
                    v-if="section.id === 'pending'"
                    :ws-status="wsStatus"
                  />
                  <!-- TODO: Refactor so it's more visible -->
            ...
            <div class="space-y-3">
              <div class="" v-for="order in section.orders"> {{ 
                order }}</div>
              ...
              <div class="space-y-3">
                <OrdersPageOrderTile
                  ...
                  @decline-order="
                    (orderId, comment) =>
                      section.id === 'pending' &&
                      handleDeclineOrder(orderId, comment)
                  "
                  @mark-ready="
                    (orderId) =>
                      section.id === 'in-progress' && handleMarkReady(orderId)
                  "
                  @notify-delay="
                    (orderId) =>
                      section.id === 'in-progress' && handleNotifyDelay(orderId)
                  "
                  @archive-order="
                    (orderId) =>
                      ...
                      (section.id === 'rejected' &&
                        handleArchiveRejectedOrder(orderId))
                  "
                />
              </div>
            </div> -->
...
const filterOrders = (ordersList: OrderData[]) => {
...
// Sort orders by date
const sortOrders = (ordersList: OrderData[]) => {
...
const totalFilteredOrders = computed(() => {
  return sectionsWithOrders.value.reduce(
    (total, section) => total + (section.orders?.length || 0),
    0
  );
});
...
const connectWebSocket = () => {
  ...
  ws.value = new WebSocket(`${wsUrl}/ws/orders/old...`);

  ws.value.onopen = () => {
    console.log(
      "✅ Zainicjowałem połączenie z serwerem WebSocket - oczekuję na zamówienia"
    );
...
watch(
  ...
  (newData) => {
    if (newData && !isLiveOrdersInitialized.value) {
      ...
      console.log(
        `✅ Wczytałem ilość zamówień: ${liveOrders.value.length} z serwera`
      );
    }
  },
  { immediate: true }
...
/* Logging section data, and checking data initialization */
onMounted(async () => {
  console.log(
    "Section data:",
    JSON.parse(JSON.stringify(sectionOrderWithData.value))
  );
  console.log("Is data initialized?", isDataInitialized.value);
});
